...
Apply the Gradle plugin and drop bundles into `src/pulsarBundles/`. On every build it generates the
typed accessor and packages the bundle into the APK assets — no manual codegen step:
...
```kotlin
val pulsar = Pulsar(context)
val bundle = pulsar.loadBundleSync(AcmePack.descriptor, strict = true)
...
```kotlin
val bundle = pulsar.loadBundleAsync(AcmePack.descriptor, strict = true)
```

`strict = true` checks the packaged bundle against the generated types, so a stale APK asset fails
loudly instead of quietly missing a preset. Omit it and the check is skipped.
...
to release the native patterns when you are done with a bundle.

See [`Android/PulsarApp`](https://github.com/software-mansion/pulsar/tree/main/Android/PulsarApp) for
a working screen.
