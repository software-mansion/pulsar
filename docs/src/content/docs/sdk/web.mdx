...
<Aside type="caution" title="No real vibration on iOS / iPadOS">
  ...
  The upside: Pulsar's audio fallback still lets users **hear** a preview of the pattern on iOS, and real haptics work on Android (Chrome, Firefox, and other Chromium-based browsers) and any browser that implements the Vibration API. For real haptics on Apple devices, use the native [iOS](/sdk/ios) or [React Native](/sdk/react-native) SDKs.
</Aside>

## Requirements

- A modern browser that supports the [Web Vibration API](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/vibrate) (`navigator.vibrate`) for real haptics — Chrome, Edge, Firefox, and Chromium-based mobile browsers. See [Browser support](#browser-support) for details.
...
<Tabs>
  <TabItem label="npm" icon="npm">
    <Code
      ...
      frame="terminal"
    />
    React is an optional peer dependency — install it only if you use the `pulsar-haptics/react` hooks.
  </TabItem>
  <TabItem label="CDN">
...
A collection of ready-to-use haptic patterns — the simplest way to add feedback. The package ships a `Presets` singleton you can use directly:
...
For one-shot haptics that aren't covered by a preset, build your own pattern with `PatternComposer`. Get a fresh composer from a `Pulsar` instance (or construct one directly), `parse()` a pattern, then `play()` it.
