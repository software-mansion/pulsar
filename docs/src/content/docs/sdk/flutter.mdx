...
Begins realtime playback if it isn't already running and updates the haptic with the new amplitude and frequency. Values should be in the 0–1 range. Playback continues until `stop()` is called or until no `set(...)` arrives within a short keepalive window (~400 ms), after which it stops automatically.
...
// The first set(...) begins playback automatically.
await realtime.set(0.5, 0.8);
await realtime.set(1.0, 0.3);

await realtime.playDiscrete(0.7, 0.5);

// End the continuous haptic.
await realtime.stop();
```

<Aside type="note" title="Lifecycle">
  `set(...)` begins playback automatically and `stop()` ends it. Playback also auto-stops shortly after the last `set(...)` via a built-in keepalive, so a gesture or sensor stream that goes silent won't leave the haptic running. Hold an input perfectly still (no `set(...)`) for longer than the keepalive and playback simply lapses until the next `set(...)`.
</Aside>
