...
### Add the bundle and generate

Declare the directory once — new bundles then need no extra entry:
...
final pulsar = Pulsar();
final bundle = await pulsar.loadBundleAsync(acmePack);

bundle.heartbeatV2.play();
bundle.explosion.stop();
```

The generated descriptor already knows the asset path, so `loadBundle` finds and loads it.
The shipped bundle's content hash is checked against the generated types; pass `strict: false`
to skip it.
...
See [`flutter/PulsarApp`](https://github.com/software-mansion/pulsar/tree/main/flutter/PulsarApp)
