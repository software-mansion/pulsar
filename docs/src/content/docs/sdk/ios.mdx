...
Add the `.pulsar` to your app target (Copy Bundle Resources), then either apply the SwiftPM build
...
```swift
let pulsar = Pulsar()
let bundle = try pulsar.loadBundleSync(AcmePack.descriptor, strict: true)

bundle.heartbeatV2.play()
bundle.explosion.stop()
```

`loadBundleAsync` is the same load with the file read off the calling thread:

```swift
let bundle = try await pulsar.loadBundleAsync(AcmePack.descriptor, strict: true)
```

The descriptor finds the `.pulsar` in the app's main bundle. `strict: true` checks the shipped
bundle against the generated types, failing loudly on a stale copy.
...
if let animation = bundle.heartbeatV2.animation {
    myLottieView.load(data: animation.data)
}
```

Audio authored into a preset is registered as a Core Haptics audio event, so it shares the engine
clock with the haptics.
...
See [`iOS/PulsarApp`](https://github.com/software-mansion/pulsar/tree/main/iOS/PulsarApp) for a
working screen.
