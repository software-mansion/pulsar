...
LaunchedEffect(Unit) {
    ...
    bundle = pulsar.loadBundleSync(AcmePack.descriptor, bytes, strict = true)
}
...
```kotlin
bundle = pulsar.loadBundleAsync(AcmePack.descriptor, bytes, strict = true)
```

`strict = true` checks the loaded bundle against the generated types.
