...
```kotlin
fun parsePattern(pattern: PatternData, fromMs: Long = 0)
```
...
```kotlin
fun parsePatternWithSound(pattern: PatternData, sound: SoundData, fromMs: Long = 0)
```
...
KMP ships the bundle API under `com.swmansion.pulsar.kmp.bundle`, so point the generator at that
runtime package:
...
KMP has no shared asset API, so the app supplies the bytes. With Compose Resources, put the bundle
in `commonMain/composeResources/files/` and read it in a `LaunchedEffect`:

```kotlin
val pulsar = Pulsar.create()
var bundle by remember { mutableStateOf<AcmePack.Presets?>(null) }

LaunchedEffect(Unit) {
    val bytes = Res.readBytes("files/acme-pack.pulsar")
    bundle = pulsar.loadBundleSync(AcmePack.descriptor, bytes)
}

bundle?.heartbeatV2?.play()
```

`loadBundleAsync` is the `suspend` equivalent, decoding on `Dispatchers.Default`:

```kotlin
bundle = pulsar.loadBundleAsync(AcmePack.descriptor, bytes)
```

If the `.pulsar` ships as a platform asset — `src/androidMain/assets/` on Android, the app bundle
on iOS — the descriptor resolves it by name and no byte-reading is needed:

```kotlin
val bundle = pulsar.loadBundleSync(AcmePack.descriptor)
```

Compose Resources stores files under its own directory rather than the platform asset location, so
a bundle kept there uses the `bytes` overload above.
...
See [`kmp/PulsarApp`](https://github.com/software-mansion/pulsar/tree/main/kmp/PulsarApp) for a
working screen.
