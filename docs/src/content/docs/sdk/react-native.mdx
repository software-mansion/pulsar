...
A `.pulsar` bundle exported from Pulsar Studio holds your own presets — each a haptic pattern with
optional synced audio and animation. Run the generator once and every preset becomes a typed member
...
That scans `./assets` for `*.pulsar` files and writes a `*.bundle.ts` next to each — that
generated module is what your app imports. On Expo, add the config plugin instead and it
regenerates on every prebuild:

```json
{ "expo": { "plugins": [["react-native-pulsar", { "bundleDirs": ["assets"] }]] } }
```

The generated module statically requires its own `.pulsar`, so register the extension with Metro
once:
...
export const Haptics = loadBundleSync();
```
...
Presets are direct members of the bundle, alongside its own `id`, `contentHash`, `get` and
`dispose`. A preset can never collide with those — the generator rejects a bundle that reuses one
of those names — and only the presets are enumerable, so `Object.keys(Haptics)` lists the pack.

Each preset exposes `play()`, `stop()`, its `id`, `name`, `duration`, the raw `pattern`, and flags
describing how it was authored:

```ts
Haptics.fanfare.hasAudio       // true if the preset has a synced sound
Haptics.fanfare.hasAnimation   // true if it has a Lottie animation
Haptics.fanfare.animation      // the Lottie itself, when the bundle carries it in JS
```

### Playing a preset's animation

A preset authored with an animation can drive `react-native-pulsar-lottie` on its own — the view
takes the animation and the haptics from the preset, with no `source` or `haptics` prop:
...
`loadBundleSync()` is synchronous and never touches the `.pulsar` binary — it plays the patterns
embedded in the generated module, parsing each lazily on first `play()`. Preset names autocomplete
and a typo is a compile error: the types come from the generated module itself, so renaming a preset
in Studio and re-generating turns every stale reference red.

Use `Haptics.get(id)` when the preset id is only known at runtime; it returns `undefined` for an
unknown id rather than throwing.

### Presets that need the .pulsar file

Two kinds of preset cannot be served by the embedded patterns alone: one with **synced audio**, and
one whose animation was exported as a **dotLottie** (`.lottie`) rather than JSON. The generator warns
about both, and `hasAudio` / `hasAnimation` identify them at runtime — such a preset still plays on
the sync path, but only its haptics.

To get the sound or the dotLottie, load the binary instead:

```ts
import { loadBundleAsync } from '@/assets/my-pack.bundle';

const bundle = await loadBundleAsync();

bundle.celebration.play();   // haptics + audio
```

Native code reads the `.pulsar` before the promise settles; after that, `play()` is synchronous
either way. `loadBundleSync(true)` reads the binary on the calling thread instead — a local file
read in release, but a **blocking Metro HTTP round trip in development**, so prefer the async loader
unless you genuinely cannot await.

One generated module drives both paths, so the types are identical either way. Loading the same pack
twice gives two independent bundles; `dispose()` on one leaves the other working, and a disposed
bundle is inert rather than silently re-parsing.

### Playing from a position

Pass `fromMs` to start a preset that far into its own timeline, so a progress bar can scrub it:

```ts
bundle.celebration.play(2500);   // start 2.5s in
bundle.celebration.play();       // ...and from the top
```

The native side re-anchors the pattern and seeks the audio to match, so both move together. On the
embedded-pattern path there is no audio to move, so a seek there scrubs the haptics alone. Every
...
| `Settings.getHapticCapabilities()`               | Returns the individual `HapticCapabilities` behind that level |
...
`HapticSupport` collapses the hardware into one ordered level, which is the right check for
"how rich a pattern can this device play". `Settings.getHapticCapabilities()` returns the
individual platform capabilities behind that level, for the cases where the level is not
enough.

```ts
import { Settings } from 'react-native-pulsar';

const caps = Settings.getHapticCapabilities();
```

| Field                           | Description                                                        |
| ------------------------------- | ------------------------------------------------------------------ |
| `hasAmplitudeControl`           | Amplitude (intensity) control, rather than on/off buzzing only     |
| `hasPrimitiveSupport`           | `VibrationEffect.Composition` primitives (Android API 30+)         |
| `isEnvelopeSupported`           | Envelope effects (Android API 36+)                                 |
| `isFrequencyProfileSupported`   | A vendor frequency profile is available (Android API 36+)          |
| `minControlPointDurationMillis` | Shortest envelope control point the device will honor, in ms       |
...
On iOS, Core Haptics exposes a single `supportsHaptics` capability and the Taptic Engine
renders the full event set uniformly, so `hasAmplitudeControl`, `hasPrimitiveSupport` and
`isEnvelopeSupported` all mirror it; `isFrequencyProfileSupported` is `false` and
`minControlPointDurationMillis` is `0`.
...
const { hasPrimitiveSupport } = Settings.getHapticCapabilities();

if (!hasPrimitiveSupport) {
  Settings.enableImpulseCompositionMode(false);
}
```
