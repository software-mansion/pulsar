...
That scans `./assets` for `*.pulsar` files and writes a `*.bundle.json` next to each — that
generated file is what your app imports. On Expo, add the config plugin instead and it regenerates
on every prebuild:
...
import myPack from '@/assets/my-pack.bundle.json';

export const Haptics = createBundle(myPack);
```
...
`createBundle` is synchronous and needs no Metro configuration. Preset names autocomplete and a
typo is a compile error — the types come from the generated file itself, so renaming a preset in
Studio and re-generating turns every stale reference red.
...
Two kinds of preset cannot be served by the generated file alone: one with **synced audio**, and one
whose animation was exported as a **dotLottie** (`.lottie`) rather than JSON. The generator warns
about both, and `hasAudio` / `hasAnimation` identify them at runtime — such a preset still plays,
but only its haptics.

To get the sound or the dotLottie, ship the `.pulsar` alongside. Register the extension with Metro:
...
...then bind the same generated file to it and load:

```ts
import { createBundleFromAsset, loadBundle } from 'react-native-pulsar';

const descriptor = createBundleFromAsset(myPack, require('@/assets/my-pack.pulsar'));
const bundle = await loadBundle(descriptor);

bundle.celebration.play();   // haptics + audio
```

The generated file drives both paths, so the types are identical either way.
...
// Composed primitives are dropped silently when unsupported, so pick a
// waveform-based pattern on those devices instead.
