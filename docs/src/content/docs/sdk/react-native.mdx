...
Begins realtime playback if it isn't already running and updates the haptic with the new amplitude and frequency. Playback continues until `stop()` is called or until no `set(...)` arrives within a short keepalive window (~400 ms), after which it stops automatically. Worklet-safe.
