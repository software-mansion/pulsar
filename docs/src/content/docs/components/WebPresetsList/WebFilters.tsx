...
/**
 * Tag-only filter bar for the web presets playground — same chrome as the mobile
 * Filters component, minus the iOS/Android system-preset selector.
 */
export function WebFilters({ selectedTags, setSelectedTags }: Props) {
    ...
    <div className={style.filters}>
      ...
      <div className={style.tagsBar}>
        ...
        {selectedTags.length > 0 && (
          <Tag label="Clear all" className={style.tagMargin} onClick={() => setSelectedTags([])} />
        )}
      </div>
    </div>
  );
