 ...
 * candy pops, so the finale can be scheduled rather than discovered.
 */
 ...
 *
 * Played honestly, a striped candy turns up about once every two moves for
...
/**
 * Chance that a refilled candy copies a neighbour's colour — see
...
/**
 * How far, as a fraction of a cell, a candy must be pushed before the swap
 * commits. Low enough to feel eager, high enough that a tap with a shaky finger
 * stays a tap.
 */
const COMMIT_AT = 0.42;

/** The grabbed candy never slides further than this, so it reads as a nudge. */
 ...
 * player already pushed. `REJECT_HOLD_MS` is the beat the candy visibly resists
 * before sliding home.
 */
 ...
 * recoil by the same *proportion*, so the candy never swings out of its own
...
/**
 * A live pixel offset applied to two neighbouring candies.
 *
 * Both the drag and the illegal-move wiggle use it, which is the point: a
 * rejected swap must never touch the board. Moving candies by *offset* leaves
 ...
 * neighbouring candies — never gets interrupted and reversed mid-flight.
 */
type Nudge = {
  from: number;
  to: number;
...
/**
 * One candy. Memoised because a drag re-renders the board on every pointermove
 * and only the two candies being pushed actually change — without this, all 64
...
const Tile = memo(function Tile({
  ...
  hinted: boolean;
  /** The candy under the finger — it lifts and rides above the board. */
  grabbed: boolean;
  ...
  return (
    <div
      className={[
        'candy',
        clearing ? 'candy--clearing' : '',
        selected ? 'candy--selected' : '',
        hinted ? 'candy--hint' : '',
        grabbed ? 'candy--grabbed' : '',
        displaced ? 'candy--displaced' : '',
        rejecting ? 'candy--rejecting' : '',
        tile.special !== 'none' ? 'candy--special' : '',
      ]
        .filter(Boolean)
        .join(' ')}
      style={
    ...
    >
      <CandySprite color={tile.color} special={tile.special} />
    </div>
  );
});
...
export function CandyCascade() {
   ...
   * because the board must keep `overflow: hidden` for the candies' drop-in and
   ...
   *
   * Offsets are never dropped to zero in one step: doing that hands the candy
  ...
  const playStep = useCallback(
    async (step: CascadeStep, before: Board, run: number) => {
      ...
      // Mark the doomed candies so CSS can pop them, then fire their effects.
      ...
      step.clear.created.forEach((entry) => {
        window.setTimeout(() => {
          ...
          // does — a striped candy is useless to a player who has not worked
  ...
  const playMove = useCallback(
    async (from: number, to: number) => {
      ...
      // An illegal swap leans the two candies into each other and lets them
      ...
      if (!result) {
        ...
        // Inherit the offset the drag already built and just hold it: the candy
      ...
      swapEffect();
      // Cleared together with the board change: the candy is already part-way
  ...
  function onPointerDown(event: React.PointerEvent<HTMLDivElement>) {
    ...
    // the gesture — that is what turns a candy grab into a smeared highlight.
  ...
  /**
   * A gesture pushes one candy against one neighbour.
   *
   ...
   * of letting the candy walk along the row under the cursor.
   */
  ...
  function onPointerUp() {
    ...
    // candy remains selected and tap-then-tap still works.
    endDrag();
  }
  ...
  return (
    <>
      ...
      <div
        ...
        role="application"
        aria-label="Candy board"
      >
        {board.map((tile, index) => {
          if (!tile) return null;

          // The grabbed candy follows the finger; the one it is pushing against
        ...
        {combo && combo.level >= 2 && (
          <div
            ...
            key={combo.level}
            className={`candy-combo${combo.exiting ? ' candy-combo--out' : ''}`}
            style={{ '--heat': `${Math.min(1, (combo.level - 2) / 6)}` } as CSSProperties}
            aria-live="polite"
          >
            <span className="candy-combo__value">&times;{combo.level}</span>
            <span className="candy-combo__label">combo</span>
          </div>
        )}

        {banner && (
          <div className={`candy-banner candy-banner--${banner.kind}`} key={banner.id}>
            {banner.kind === 'super' && <span className="candy-banner__wave" aria-hidden="true" />}
            <span className="candy-banner__title">{banner.title}</span>
            <span className="candy-banner__detail">{banner.detail}</span>
          </div>
        )}

        {over && (
          <div className="candy-over">
            <span className="candy-over__title">Out of moves</span>
            <span className="candy-over__score">{score.toLocaleString()} points</span>
      ...
      {shell && createPortal(<canvas ref={canvasRef} className="candy-particles" />, shell)}

      <p className="hint candy-footnote">
        {hapticsAvailable()
