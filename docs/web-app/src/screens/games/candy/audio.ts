...
class GameAudio {
  ...
  /**
   * The candy pop. Cascade level walks up the pentatonic scale, so a chain
  ...
  /** Candies hitting the bottom: a handful of dry wood-block clicks. */
  ...
  /** A striped candy's beam crossing the board. */
  sweep() {
    const t = this.now();
    if (t === null) return;
  ...
  /** A special candy being created — a bright upward arpeggio. */
  born(steps: number) {
    const t = this.now();
    if (t === null) return;
