...
### Candy Cascade (`screens/games/candy/`)
...
| `CandyCascade.tsx` | Playback only: walks the steps the engine returned and fires effects.   |
...
five deep _before_ the first candy pops, so the finale can be scheduled instead
of discovered.
...
wipeout. The shared shell lives on `.candy-banner`; each kind overrides only the
...
`overflow: hidden` for the candies' drop-in, which would slice every spark off
