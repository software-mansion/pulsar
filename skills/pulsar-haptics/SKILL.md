...
description: >
  ...
  Trigger on explicit mobile-haptic terms such as vibration, buzz, taptic, rumble,
  motor feedback, tactile feedback, haptic pulse, UIFeedbackGenerator, CHHapticEngine,
  or VibrationEffect, even when Pulsar is not named.
...
2. Prefer a preset for ordinary events. Use a custom pattern when the user explicitly requests one, requires exact timing or an evolving envelope, or no preset fits.
3. Select the smallest set of resources needed to complete the task:
